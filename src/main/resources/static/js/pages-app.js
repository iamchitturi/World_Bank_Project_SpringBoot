...
async function pgDash() {
  ...
  const accs = accounts.map(a => `<div class="glass-panel rounded-xl p-6 shadow-soft hover:shadow-md transition-shadow border border-outline-variant/30 flex flex-col relative overflow-hidden">
    ...
    <div class="mt-auto"><div class="text-2xl font-semibold text-on-surface mb-4 live-indicator inline-block pr-3">₹${fmt(a.balance)}</div>
    <div class="flex space-x-3 border-t border-outline-variant/50 pt-4"><button class="text-xs font-semibold text-primary flex items-center hover:underline btn-interactive" onclick="showModal('deposit')">Deposit <span class="material-symbols-outlined text-[16px] ml-1">arrow_forward</span></button><span class="w-px bg-outline-variant h-4 my-auto"></span><button class="text-xs font-semibold text-on-surface-variant hover:text-on-surface btn-interactive" onclick="showModal('withdraw')">Withdraw</button></div></div></div>`).join('');
  ...
  main.innerHTML = `<header class="flex flex-col md:flex-row md:items-end justify-between mb-8 pb-8 border-b border-outline-variant">
    <div><h1 class="text-3xl font-display font-bold text-gradient mb-2 animate-float" style="animation-duration: 6s; display: inline-block;">Good morning, ${escapeHTML(user.name)}</h1><p class="text-base text-on-surface-variant">Here is a summary of your accounts.</p></div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
    <div class="lg:col-span-2 glass-panel rounded-xl p-6 shadow-soft border border-outline-variant/30 card-lift">
      <h3 class="text-xl font-display font-semibold text-primary mb-4 text-gradient">Balance Trend</h3>
  ...
  <div class="flex justify-between items-end mb-6"><h3 class="text-xl font-display font-semibold text-primary text-gradient">Recent Transactions</h3><a href="#history" class="text-xs font-semibold text-primary hover:underline flex items-center">View All <span class="material-symbols-outlined text-[16px] ml-1">arrow_forward</span></a></div>
  <div class="glass-panel rounded-xl shadow-soft border border-outline-variant/30 overflow-x-auto w-full" id="recentTxn"><div class="p-4">${skelRow.repeat(3)}</div></div>`;
...
async function pgCards() {
  ...
  await loadAccounts(); await loadCards();
  const rows = cards.length ? `<table class="w-full text-left whitespace-nowrap"><thead><tr class="bg-surface-container border-b border-outline-variant"><th class="py-3 px-6 text-xs font-semibold text-on-surface-variant uppercase">Card</th><th class="py-3 px-6 text-xs font-semibold text-on-surface-variant uppercase">Type</th><th class="py-3 px-6 text-xs font-semibold text-on-surface-variant uppercase">Expiry</th><th class="py-3 px-6 text-xs font-semibold text-on-surface-variant uppercase">Status</th><th class="py-3 px-6 text-xs font-semibold text-on-surface-variant uppercase">Action</th></tr></thead><tbody class="divide-y divide-outline-variant/50">${cards.map(c => `<tr class="hover:bg-surface-container-low"><td class="py-3 px-6 text-sm font-semibold">${escapeHTML(c.maskedNumber)}</td><td class="py-3 px-6 text-sm">${escapeHTML(c.cardType)}</td><td class="py-3 px-6 text-sm">${escapeHTML(c.expiryDate)}</td><td class="py-3 px-6 text-sm"><span class="px-2 py-1 rounded-full text-[10px] font-bold ${c.status==='ACTIVE'?'bg-green-100 text-green-800 live-indicator inline-block':'bg-red-100 text-red-800'}">${escapeHTML(c.status)}</span></td><td class="py-3 px-6">${c.status === 'ACTIVE' ? `<button class="px-4 py-1.5 bg-error text-white rounded text-xs font-semibold hover:bg-red-800 transition-colors btn-interactive" onclick="blockCard(${c.id})">Block</button>` : `<button class="px-4 py-1.5 bg-primary text-white rounded text-xs font-semibold hover:bg-primary-container transition-colors btn-interactive" onclick="activateCard(${c.id})">Activate</button>`}</td></tr>`).join('')}</tbody></table>` : '<p class="p-6 text-on-surface-variant">No cards linked.</p>';
  document.getElementById('app').innerHTML = shell('cards', `<div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8 gap-4"><h1 class="text-3xl font-display font-bold text-gradient">Cards</h1><button onclick="showIssueCard()" class="px-5 py-2 bg-primary text-white rounded-lg text-sm font-semibold hover:bg-primary-container transition-colors shadow-sm btn-interactive">+ Apply for Card</button></div><div class="glass-panel rounded-xl shadow-soft border border-outline-variant/30 overflow-x-auto w-full">${rows}</div>`);
}
...
async function pgTransfer() {
  ...
  document.getElementById('app').innerHTML = shell('transfer', `<h1 class="text-3xl font-display font-bold text-gradient mb-8">Fund Transfer</h1>
  <div class="glass-panel rounded-xl p-8 shadow-soft border border-outline-variant/30 max-w-lg card-lift"><form id="tf" class="space-y-5">
    ...
    <button type="submit" class="w-full py-3 bg-primary text-white rounded-lg text-sm font-semibold hover:bg-primary-container shadow-sm transition-colors btn-interactive">Initiate Transfer</button>
  </form></div>`);
